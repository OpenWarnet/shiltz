...
// tables plus the per-class constants in Stats.cpp. Purely an in-memory
// cache -- everything it reads is already persisted or static data, so
// it's fully reproducible and never gets its own DB column. Call after
// any trigger that could change an input: today that's a successful
