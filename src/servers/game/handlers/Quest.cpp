                                               ...
                                               const QuestConsequences& q, const WarpRecord* warp)
{
    ...
    if (warp)
    {
        ...
        character.placement.x = static_cast<std::uint32_t>(warp->x);
        character.placement.y = static_cast<std::uint32_t>(warp->y);
        CharacterRepository::SavePosition(db, characterId, character.map_id,
